...
# usage: `./draw_motif.sh <plotstr>` generates pdf for the given plotstr

# Check if the environment variable is not set
if [ -z "${VIENNA}" ]; then
    # report error and exit
    echo "Error: VIENNA environment variable is not set."
    exit 1
fi
echo "\$VIENNA: $VIENNA"
mode=0
...
if [ "$result_cross" -eq 1 ]; then
    echo "cross"
    # the current ps file is not needed anymore, will be updated right after the following line
    rm ${id}_ss.ps
